...
#define SIZE 100
#define TESTING 0
#define TEST_RESULTS 1 // this displays (to console) the results of each iteration within the TSP_NN_PRIMS alg
...
struct TSPSolution TSP_NN_Prims(int **distanceMatrix, int n) 
{
    // initialize struct
...
int main(int argc, const char * argv[]) {
    ...
    if(!argv[1]) {    //test algorithms 2-3
        cout << "usage: " << argv[0] << " <input file name> or " << argv[0] << " <path\\to\\your\\file.ext>\n\n";
        exit(1);
    }
    else inputFile = inputName = argv[1];
    ...
    soln = TSP_NN_Prims(mapMatrix, numCities);
